...
echo ">>> TRUMETRICS RESTORE PROTOCOL INITIATED..."
...
# 2. WRITE CONFIGURATION FILES (Safe text, no complex symbols)
echo ">>> Writing Web Configurations..."
...
export default {
  ...
  theme: { extend: { fontFamily: { mono: ['Menlo', 'monospace'] } } },
  plugins: [],
}
EOF
...
<!doctype html>
<html lang="en">
  <head><meta charset="UTF-8" /><title>TruMetrics Empire</title></head>
  <body class="bg-slate-950"><div id="root"></div><script type="module" src="/src/main.tsx"></script></body>
</html>
EOF
...
# 3. WRITE THE COMPLEX COMPONENT (Using Node Generator to bypass Shell issues)
echo ">>> Generating Schematic Assets (Safe Mode)..."

cat << 'JS_EOF' > apps/web/generate_anatomy.js
const fs = require('fs');
...
fs.writeFileSync('apps/web/src/components/schematics/HomeAnatomy.tsx', code);
console.log(">>> COMPONENT GENERATED.");
JS_EOF

# Execute the generator
node apps/web/generate_anatomy.js

echo ">>> RESTORE COMPLETE."
echo ">>> RUN 'cd apps/web && npm install && npm run dev' TO LAUNCH."
